...
actor {
    ...
    public type Microblog = actor {
        ...
        post: shared (Text, Text) -> async (); //发布消息
        posts: shared query (Time.Time) -> async [Message]; //获取发布的消息
        timeline: shared ([Principal], Time.Time) -> async [Message]; //获取所有关注的人发布的消息
        set_name: shared (Text, Text) -> async ();  // 设置作者名称
        get_name: shared query () -> async ?Text;  // 获取作者名称
    };
